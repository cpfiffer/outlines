...
You'll need to install the following packages to get started:
...
If you are hardware constrained, you may wish to load a quantized model or use the CPU. Transformers supports [4 and 8 bit quantization](https://huggingface.co/docs/transformers/main/en/quantization/bitsandbytes). Install the `bitsandbytes` package:

```bash
pip install bitsandbytes
```

Next, load the model in 4-bit mode:
...
Once the model is initialized, you can call `outlines.generate` to construct a [generator](https://wiki.python.org/moin/Generators). This generator can be called with a prompt directly as if it were a function, i.e. `generator("How do I bake a cake?")`. `outlines.generate` is a drop-in wrapper for inference on the underlying model.

The simplest generator is `outlines.generate.text`, which produces standard unstructured text.

```python
# Construct our generator from the language model above
generator = outlines.generate.text(model)

# Now we provide a prompt and limit the number of tokens generated
result = generator("Question: What's 2+2? Answer:", max_tokens=100)
print(result)
# The answer is 4
```

If you prefer to generate tokens inside an iterator, you can use the `generator.stream` method:

```python
# Outlines also supports streaming output
n_tokens = 5
stream = generator.stream("What's 2+2?", max_tokens=n_tokens)
...
The real power of Outlines is structured text generation, which limits output to a specific format.
...
See the [Structured Generation](./reference/generation/generation.md) documentation for more detail.

A simple example is using [pydantic](https://docs.pydantic.dev/latest/) to define a structured output:

```python
from enum import Enum
from pydantic import BaseModel, constr, conint

# Define a Character model with pydantic
class Character(BaseModel):
    # The character's name can be up to 10 characters long
    name: constr(max_length=10)

    # The character's age must be between 18 and 99
    age: conint(gt=18, lt=99)

    # The character's armor can be leather, chainmail, or plate
    armor: (Enum('Armor', {'leather': 'leather', 'chainmail': 'chainmail', 'plate': 'plate'}))

    # The character's strength must be between 1 and 100
    strength: conint(gt=1, lt=100)
```

Pydantic models can be converted to [a JSON schema](https://docs.pydantic.dev/latest/why/#json-schema). Outlines handles conversion under the hood when you pass a pydantic model to `outlines.generate.json`:

```python
# Note that we are using generate.json instead of generate.text
